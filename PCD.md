...
**功能描述**
- 历史记录由“高频快照”调整为“精简版本点”：在 3 分钟窗口内，若内容差异较小且相似度高，则合并为同一条记录，减少重复历史。

**用户故事**
- 作为写作者，我希望历史列表展示关键版本，而不是每几秒一次的近似重复草稿，便于快速定位可恢复节点。

**设计稿链接**
- N/A（本次仅涉及保存策略与数据逻辑，无新增界面布局改动）。

**技术方案**
- 在 `saveHistoryEntry` 中引入时间窗口与内容相似度合并规则：
  - 时间窗口：最近 3 分钟；
  - 差异阈值：文本长度变化小于 20；
  - 相似度阈值：前缀字符相似度不低于 0.92。
- 命中合并条件时更新最新条目内容与时间戳；未命中则新增历史条目。

**测试用例**
- 窗口内轻微编辑：应合并为 1 条记录。
- 窗口外编辑：应新增为 2 条记录并保持时间倒序。

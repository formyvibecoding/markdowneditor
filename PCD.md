...
**功能描述**
- 历史记录展示方式由右侧抽屉改为居中模态弹窗，默认关闭，点击页脚入口后再打开。
- 所有运行时提示统一替换为应用内组件：错误提示改为自动消失 Toast，确认类操作改为内部确认弹窗。

**用户故事**
- 作为写作者，我希望历史记录不会默认占用界面空间，只有在需要时才打开。
- 作为用户，我希望确认/错误提示与产品风格一致，不出现浏览器原生样式断层。

**设计稿链接**
- 暂无独立设计稿，沿用当前 codebase 视觉风格。

**技术方案**
- 新增 `src/feedback.ts` 统一封装 `showToast/showErrorToast/showConfirm`。
- `src/main.ts` 历史记录弹层改为 modal 交互，支持遮罩点击、ESC、关闭按钮与焦点回退。
- `src/templates/preview.ts` 内联脚本增加预览页内部 toast，替换原生 alert。

**测试用例**
- E2E：验证历史记录默认关闭、点击后显示弹窗、ESC 可关闭。
- E2E：空内容下载显示内部 Toast，不触发浏览器 dialog。
- 单元测试：保持既有历史与工具模块测试通过。
...
**功能描述**
- 新增本地历史记录功能，不依赖账户体系；草稿自动保存到浏览器本地。
- 历史记录以月/日维度汇总展示，支持恢复草稿、删除单条、清空全部。
- 页面保持极简风格，在页脚 GitHub 图标旁新增历史入口。
- 编辑器每次启动默认空白，不自动回填历史草稿。

**用户故事**
- 作为作者，我希望关闭页面后仍能在历史里找到未完成草稿。
- 作为作者，我希望下次打开编辑器仍是空白，以便快速开始新内容。
- 作为作者，我希望按日期快速回溯并恢复旧草稿。

**设计稿链接**
- 沿用当前界面风格，暂无独立设计稿（N/A）。

**技术方案**
- 新增 `history.ts` 管理本地持久化（localStorage）、分组、格式化与增删操作。
- 主入口增加历史面板渲染与交互逻辑，入口放置在页脚图标区。
- 自动保存采用 2 秒防抖 + `beforeunload` 补存，避免高频写入。

**测试用例**
- 编辑内容后关闭窗口，重新进入可在历史面板看到草稿条目。
- 历史按“月/日”分组显示，时间与摘要正确。
- 点击恢复需确认，确认后覆盖编辑区内容。
- 点击删除仅删除目标条目；清空全部后列表为空。

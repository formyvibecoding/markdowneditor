...
- 新增统一的应用内提示系统：支持自动消失 Toast 与可复用的确认弹窗

### 改进

- 历史记录面板改为居中模态弹窗，默认保持关闭，仅在用户点击入口后打开
- 历史记录弹窗支持右上角关闭、点击遮罩关闭、ESC 关闭，并补充移动端与键盘可访问性体验
- 主页面与预览页面移除浏览器原生 alert/confirm，统一使用产品内提示反馈

## [2.3.0] - 2026-02-12

### 新增

- 新增本地历史记录功能：自动保存草稿快照、按月/日分组展示、支持手动恢复与单条删除
- 页面底部新增历史记录入口图标（位于 GitHub 图标旁），打开右侧历史面板

### 改进

- 编辑器启动时保持空白，不再自动填充示例内容，符合“草稿仅在历史中查看”的使用流程
- 新增历史分组与格式化相关单元测试
...
- 复制预览内容时同步复选框勾选状态，确保粘贴到富文本编辑器时保留任务列表状态

## [2.2.0] - 2026-01-25

### 新增

- 预览页支持任务列表复选框渲染，`- [ ]` 显示为空框、`- [x]/- [X]` 显示为已勾选，并可交互切换

### 改进

- HTML 消毒规则允许安全的复选框标签与属性，确保任务列表渲染可用

## [2.1.0] - 2026-01-24

### 新增

- 新增英文版 README，覆盖功能介绍、使用方式、贡献指南等完整文档
- 预览页新增“复制预览”按钮，支持一键复制带样式的预览内容

### 改进

- 页脚增加 GitHub 项目入口图标，方便快速访问仓库
